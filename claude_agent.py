...
async def main():
    ...
    options = ClaudeAgentOptions(
        system_prompt={
            ...
            "preset": "claude_code",
            "append": (
                "\n\n## ⛔ ABSOLUTE FILE CONSTRAINTS — VIOLATIONS WASTE 30-50 SECONDS EACH\n\n"
                "PERFORMANCE: Rewriting a file = 30-50 SECONDS. Edit(targeted change) = 2-3 SECONDS.\n\n"
                "### REWRITING FILES IS FORBIDDEN\n"
                "After a file is created, you MUST NOT rewrite it — not via Write tool, not via Bash (cat >, heredoc, echo >), not via any method.\n"
                "The ONLY way to modify an existing file is the **Edit** tool (old_string → new_string).\n"
                "- ❌ FORBIDDEN: Write tool on existing file\n"
                "- ❌ FORBIDDEN: Bash `cat > file`, `cat > file << EOF`, `echo '...' > file`, heredoc, tee, or any Bash file-rewrite\n"
                "- ❌ FORBIDDEN: Write → Read/Bash head → Write/Bash cat (wastes 60-110 seconds!)\n"
                "- ✅ ONLY ALLOWED: Edit tool with targeted old_string → new_string\n\n"
                "### Bash tool restrictions\n"
                "- Bash is for commands ONLY: npm run build, git, ls, etc.\n"
                "- NEVER use Bash to write or rewrite file contents (cat >, heredoc, echo >, sed, etc.)\n"
                "- NEVER use Bash to check if a file was written correctly (head, cat, etc.) — trust your Write/Edit.\n\n"
                "### DashboardOverview.tsx\n"
                "- Write ONCE (initial creation). Then ONLY Edit for fixes. If build fails → Edit specific lines.\n\n"
                "### index.css, Layout.tsx\n"
                "- ONLY Edit. Never rewrite.\n\n"
                "### CRUD scaffold files (*Page.tsx, *Dialog.tsx, ConfirmDialog, StatCard, PageShell)\n"
                "- Do NOT touch. They are complete.\n\n"
                "### After ANY Write or Edit\n"
                "- Do NOT Read the file back. Do NOT use Bash to check it. Move to the next task.\n"
            )
        },
